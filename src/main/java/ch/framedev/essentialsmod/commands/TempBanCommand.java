...
public class TempBanCommand implements ICommand {
    ...
    private int execute(CommandContext<CommandSourceStack> command) {
        ...
        if (durationInMillis <= 0) {
            command.getSource().sendFailure(Component.literal("Invalid duration format! Use 1h, 10m, 1d, etc."));
            return 0;
        }
        ...
        if (targetPlayer != null)
            targetPlayer.connection.disconnect(Component.literal("You are banned for: " + reason + ". Remaining time: " + formatDuration(durationInMillis)));
        command.getSource().sendSystemMessage(Component.literal(playerName + " has been banned for " + formatDuration(durationInMillis) + ". Reason: " + reason));
    ...
    public static void checkBanDuringLogin(ServerLoginPacketListenerImpl loginListener, Connection connection) {
        ...
        if (banDetails != null && isPlayerBanned(loginListener.gameProfile.getId())) {
            ...
            connection.disconnect(Component.literal(banMessage));
        }
    }
