...
public class InventorySyncHandler {
    ...
    public static void onContainerClose(PlayerContainerEvent.Close event) {
        if (event.getContainer() instanceof InvseeCommand.VirtualInventoryMenu menu) {
            ServerPlayer currentPlayer = (ServerPlayer) event.getEntity(); // The player who opened the menu
            ...
            if (targetPlayer != null) {
                ...
                currentPlayer.sendSystemMessage(Component.literal("Inventory changes have been saved to " + targetPlayer.getName().getString() + "'s inventory.").withStyle(ChatFormatting.GOLD));
            } else {
                currentPlayer.sendSystemMessage(Component.literal("Could not sync inventory changes. Target player not found.").withStyle(ChatFormatting.RED));
            }
        }
    }
}