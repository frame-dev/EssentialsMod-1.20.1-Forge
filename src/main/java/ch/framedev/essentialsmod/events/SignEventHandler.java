...
public class SignEventHandler {
    ...
    public static void onSignClickRight(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (event.getLevel().getBlockEntity(event.getPos()) instanceof SignBlockEntity sign && event.getEntity() instanceof ServerPlayer serverPlayer) {
    ...
    private static void handleSignFreeEvent(SignBlockEntity sign, ServerPlayer player) {
        ...
        Component line2 = sign.getFrontText().getMessage(1, false);
        ...
        player.sendSystemMessage(Component.literal("You received " + amount + " " + itemName + "!"));
    }
